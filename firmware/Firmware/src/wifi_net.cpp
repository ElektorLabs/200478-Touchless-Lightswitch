...
void _ReconnectWiFi( bool ConnectToApOnly  ) {
  ...
  if(settings.AP_Mode == false ){
    ...
    if(0==c.ssid[0]){
      ...
      #endif
      WiFi.disconnect(false,true); //clear saved credentials, keep wifi active....
    } else {
      ssid=String(c.ssid);
      pass=String(c.pass);
...
void configureSoftAP( bool use_wifi_config ) {
  ...
  WiFi.mode(WIFI_MODE_APSTA); //Mix mode for station and AP as we need to be able to scan for new APs...
 ...
 **************************************************************************************************/
void configureServer( void ){
  /* This will setup just the basic functions */
  SetupWebServer();
